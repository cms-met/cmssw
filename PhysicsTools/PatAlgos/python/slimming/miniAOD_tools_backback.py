...
def miniAOD_customizeCommon(process):
    ...
    #corrected jets, akPFJetsCHS are already produced by the MET tool
    #from RecoJets.JetProducers.ak4PFJets_cfi import ak4PFJets
    #setattr(process, "ak4PFJets"
    #process.load("RecoJets.JetProducers.ak4PFJets_cfi")

    #process.ak4PFJets.doAreaFastjet = True # even for standard ak4PFJets this is overwritten in RecoJets/Configuration/python/RecoPFJets_cff
    #process.ak4PFJets.src=cms.InputTag("cleanMuonsPFCandidates")

    #from RecoJets.JetAssociationProducers.j2tParametersVX_cfi import j2tParametersVX
    #process.ak4PFJetsTracksAssociatorAtVertex = cms.EDProducer("JetTracksAssociatorAtVertex",
    #    j2tParametersVX,
    ...
    #configtools.cloneProcessingSnippet(process, getattr(process,"makePatJets"), "MuCor") #, noClones = noClonesTmp)
    #addJetCollection(process, postfix   = "", labelName = '', jetSource = cms.InputTag('ak4PFJetsCHS'),
    ...
    process.duplicatedPatMuons=process.patMuons.clone(
        muonSource = cms.InputTag("cloneGlobalMuonTagger:bad") )
    process.badPatMuons=process.patMuons.clone(
        muonSource = cms.InputTag("badGlobalMuonTagger:bad") )
    process.slimmedDuplicatedMuons=process.slimmedMuons.clone(
        src=cms.InputTag("duplicatedPatMuons") )
    process.slimmedBadMuons=process.slimmedMuons.clone(
        src=cms.InputTag("badPatMuons") )

    # switching packedPFCandidate collection for slimmedjets
    process.slimmedJets.packedPFCandidates = cms.InputTag("packedPFCandidatesMuClean")

    #backup jets
    process.uncorSlimmedJetBackup = cms.EDProducer("JetCollectionReducer",
    ...
    # FIXME MM -->> TO BE FIXED BEFORE PRODUCTION!!!!!!
    ...
    process.slimmedJetsPuppi.packedPFCandidates = cms.InputTag("packedPFCandidatesMuClean") #cleanMuonsPFCandidates

    
    ## puppi met
    from PhysicsTools.PatAlgos.slimming.puppiForMET_cff import makePuppies
    makePuppies( process, "cleanMuonsPFCandidates") #"cleanMuonsPFCandidates" );
    process.packedPFCandidatesMuClean = process.packedPFCandidates.clone(
        PuppiNoLepSrc = cms.InputTag("puppiNoLep"),
        PuppiSrc = cms.InputTag("puppi"),
        inputCollection = cms.InputTag("cleanMuonsPFCandidates")
        )
    ...
    #---------------------------------------------------------------------------
    #Puppi Mu/EG uncorrected 
    # Adding puppi jets

    process.ak4PFJetsPuppiMuUncorr=process.ak4PFJetsPuppi.clone(
        doAreaFastjet = True, # even for standard ak4PFJets this is overwritten in RecoJets/Configuration/python/RecoPFJets_cff
        src=cms.InputTag("particleFlow")
        )

    process.ak4PFJetsPuppiTracksAssociatorAtVertexMuUncorr = process.ak4PFJetsPuppiTracksAssociatorAtVertex.clone(
        jets = cms.InputTag("ak4PFJetsPuppiMuUncorr")
        )
    process.patJetPuppiChargeMuUncorr = process.patJetPuppiCharge.clone(
        src = cms.InputTag("ak4PFJetsPuppiTracksAssociatorAtVertexMuUncorr"),
        )

    addJetCollection(process, postfix   = "MuUncorr", labelName = 'Puppi', jetSource = cms.InputTag('ak4PFJetsPuppiMuUncorr'),
                    jetCorrections = ('AK4PFPuppi', ['L2Relative', 'L3Absolute'], ''),
                    algo= 'AK', rParam = 0.4, btagDiscriminators = map(lambda x: x.value() ,process.patJets.discriminatorSources)
                    )
    
    process.patJetGenJetMatchPuppiMuUncorr.matched = 'slimmedGenJets'
    process.patJetsPuppiMuUncorr.jetChargeSource = cms.InputTag("patJetPuppiCharge")

    process.selectedPatJetsPuppiMuUncorr.cut = cms.string("pt > 15")

    process.slimmedJetsPuppiMuUncorr = process.slimmedJets.clone()
    process.slimmedJetsPuppiMuUncorr.src = cms.InputTag("selectedPatJetsPuppiMuUncorr")    
    process.slimmedJetsPuppiMuUncorr.packedPFCandidates = cms.InputTag("packedPFCandidates")

    
    ## puppi met
    from PhysicsTools.PatAlgos.slimming.puppiForMET_cff import makePuppies
    makePuppies( process, "particleFlow", "MuUncorr" );
    process.packedPFCandidates.PuppiNoLepSrc = cms.InputTag("puppiNoLepMuUncorr")
    process.packedPFCandidates.PuppiSrc = cms.InputTag("puppiMuUncorr")
    #del process.packedPFCandidates.PuppiNoLepSrc
    #del process.packedPFCandidates.PuppiSrc

    runMetCorAndUncForMiniAODProduction(process, metType="Puppi",
                                        pfCandColl=cms.InputTag("puppiForMETMuUncorr"),
                                        jetCollUnskimmed="slimmedJetsPuppi",
                                        recoMetFromPFCs=True,
                                        jetFlavor="AK4PFPuppi",
                                        postfix="PuppiMuUncorr"
                                        )
    
    process.slimmedMETsPuppiMuUncorr = process.slimmedMETs.clone()
    process.slimmedMETsPuppiMuUncorr.src = cms.InputTag("patPFMetT1PuppiMuUncorr")
    process.slimmedMETsPuppiMuUncorr.rawVariation =  cms.InputTag("patPFMetRawPuppiMuUncorr")
    process.slimmedMETsPuppiMuUncorr.t1Uncertainties = cms.InputTag("patPFMetT1%sPuppiMuUncorr")
    process.slimmedMETsPuppiMuUncorr.t01Variation = cms.InputTag("patPFMetT0pcT1PuppiMuUncorr")
    process.slimmedMETsPuppiMuUncorr.t1SmearedVarsAndUncs = cms.InputTag("patPFMetT1Smear%sPuppiMuUncorr")
    process.slimmedMETsPuppiMuUncorr.tXYUncForRaw = cms.InputTag("patPFMetTxyPuppiMuUncorr")
    process.slimmedMETsPuppiMuUncorr.tXYUncForT1 = cms.InputTag("patPFMetT1TxyPuppiMuUncorr")
    process.slimmedMETsPuppiMuUncorr.tXYUncForT01 = cms.InputTag("patPFMetT0pcT1TxyPuppiMuUncorr")
    process.slimmedMETsPuppiMuUncorr.tXYUncForT1Smear = cms.InputTag("patPFMetT1SmearTxyPuppiMuUncorr")
    process.slimmedMETsPuppiMuUncorr.tXYUncForT01Smear = cms.InputTag("patPFMetT0pcT1SmearTxyPuppiMuUncorr")
    del process.slimmedMETsPuppiMuUncorr.caloMET
