...
from CommonTools.PileupAlgos.PhotonPuppi_cff        import setupPuppiPhoton,setupPuppiPhotonMiniAOD,puppiPhoton

def makePuppies( process, pfCandidateCollection="particleFlow", postfix="" ):
    
    setattr(process, "pfNoLepPUPPI"+postfix, 
            cms.EDFilter("PdgIdCandViewSelector",
                         src = cms.InputTag(pfCandidateCollection), 
                         pdgId = cms.vint32( 1,2,22,111,130,310,2112,211,-211,321,-321,999211,2212,-2212 )
                         )
            )
    setattr(process, "pfLeptonsPUPPET"+postfix, 
            cms.EDFilter("PdgIdCandViewSelector",
                         src = cms.InputTag(pfCandidateCollection),
                         pdgId = cms.vint32(-11,11,-13,13),
                         )
            )
    setattr(process, "puppi"+postfix, puppi.clone(
            candName=cms.InputTag(pfCandidateCollection)))
    setattr(process, "puppiNoLep"+postfix, puppi.clone(
            candName=cms.InputTag('pfNoLepPUPPI'+postfix) ))
    setattr(process, "puppiMerged"+postfix, 
            cms.EDProducer("CandViewMerger",
                           src = cms.VInputTag( 'puppiNoLep','pfLeptonsPUPPET')
                           )
            )
    setattr(process, "puppiForMET"+postfix, puppiPhoton.clone(
            useRefs=False,
            puppiCandName="puppiMerged"+postfix
            ) )
    ...
    #process.puppi.candName=cms.InputTag(pfCandidateCollection)
